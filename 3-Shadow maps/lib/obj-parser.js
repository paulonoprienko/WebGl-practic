...
function onReadMTLFile(fileString, mtl) {
  ...
  while ((line = lines[index++]) != null) {
	...
	case 'map_Kd': //
      var texPath = mtl.parseTexPath(sp);    // Get tex path
      mtl.texPaths.push(texPath);
      continue; // Go to the next line
    }
  }
  mtl.complete = true;
...
OBJDoc.prototype.getDrawingInfo = function() {
  ...
  for(var i = 0; i < this.objects.length; i++){
    ...
    vertices.push( new Float32Array(numVertices * 3) );
    normals.push( new Float32Array(numVertices * 3) );
    colors.push( new Float32Array(numVertices * 4) );
	textureCoords.push( new Float32Array(numVertices * 2) );
    indices.push( new Uint16Array(numIndices) );
  }
  var objs = [];
  ...
  for(var i = 0; i < this.objects.length; i++){
    ...
    for(var j = 0; j < object.faces.length; j++){
      ...
      for(var k = 0; k < face.vIndices.length; k++){
        // Set index
        indices[i][index_indices] = index_indices;
        // Copy vertex
        var vIdx = face.vIndices[k];
        var vertex = this.vertices[vIdx];
        vertices[i][index_indices * 3 + 0] = vertex.x;
        vertices[i][index_indices * 3 + 1] = vertex.y;
        vertices[i][index_indices * 3 + 2] = vertex.z;
        // Copy color
        colors[i][index_indices * 4 + 0] = color.r;
        colors[i][index_indices * 4 + 1] = color.g;
        colors[i][index_indices * 4 + 2] = color.b;
        colors[i][index_indices * 4 + 3] = color.a;
		// Copy textureCoord
		var tIdx = face.tIndices[k];
		var textureCoord = this.textureCoords[tIdx];
		textureCoords[i][index_indices * 2 + 0] = textureCoord.s;
		textureCoords[i][index_indices * 2 + 1] = textureCoord.t;
        // Copy normal
        var nIdx = face.nIndices[k];
        if(nIdx >= 0){
          var normal = this.normals[nIdx];
          normals[i][index_indices * 3 + 0] = normal.x;
          normals[i][index_indices * 3 + 1] = normal.y;
          normals[i][index_indices * 3 + 2] = normal.z;
        }else{
          normals[i][index_indices * 3 + 0] = faceNormal.x;
          normals[i][index_indices * 3 + 1] = faceNormal.y;
          normals[i][index_indices * 3 + 2] = faceNormal.z;
        }
        index_indices ++;
      }
    }
	index_indices = 0;
	objs.push( new DrawingInfo(vertices[i], normals[i], colors[i], textureCoords[i], indices[i]) );
  }
...
MTLDoc.prototype.parseTexPath = function(sp) {
  return sp.getWord();        
}
...
// DrawInfo Object
//------------------------------------------------------------------------------
var DrawingInfo = function(vertices, normals, colors, textureCoords, indices) {
