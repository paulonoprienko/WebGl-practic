...
var FSHADER_SOURCE = 
	...
	// определение цвета
	// '	gl_FragColor = vec4( (specular + diffuse + specular2 + diffuse2 + ambient) * visibility , color.a);\n' +
	'	gl_FragColor = vec4( (specular + diffuse * visibility + specular2 + diffuse2 + ambient) , color.a);\n' +
	'}\n';
...
window.onload = function main()
{
	...
	var light_x = 5, light_y = 10, light_z = 10;
	...
	gl.uniform3f(normalProgram.u_LightColor, 0.5, 0.5, 0.5);  // цвет позиционого источника света
	gl.uniform3f(normalProgram.u_LightPosition, light_x, light_y, light_z); // координаты положения позиционого источника света
	gl.uniform3f(normalProgram.u_AmbientLight, 0.5, 0.5, 0.5); // цвет фонового освещения
	...
	directional.oninput = function(){
		val = this.value;
		gl.uniform3f(normalProgram.u_DirLightColor, val, val, val);
	}
