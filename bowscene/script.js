...
window.onload = function main()
{
	...
	readOBJFile('bowscene.obj');
	// readOBJFile('bowling_ball_pin.obj');
	...
	{	
		...
		modelMatrix.setTranslate(5.0, 1.3, -4.0).rotate(currentAngle*10, 0, 1, 0).translate(-0.2, 0, 0); //Масштабировать, повернуть, переместить фигуру
		...
		if(program.name == "normalProgram"){
			gl.texImage2D(
				gl.TEXTURE_2D, 0, gl.RGBA, gl.RGBA,
				gl.UNSIGNED_BYTE,
				document.getElementById('pintex')
			);
			gl.uniform1i(u_Sampler, textureUnit);
			normalMatrix.setInverseOf(modelMatrix).transpose();
			gl.uniformMatrix4fv(program.u_NormalMatrix, false, normalMatrix.elements)
		...
		modelMatrix.setTranslate(1.0, 0.0, -3.0); //Масштабировать, повернуть, переместить фигуру
		gl.uniformMatrix4fv(program.u_ModelMatrix, false, modelMatrix.elements);
		if(program.name == "normalProgram"){
			normalMatrix.setInverseOf(modelMatrix).transpose();
			gl.uniformMatrix4fv(program.u_NormalMatrix, false, normalMatrix.elements)
		...
		modelMatrix.setTranslate(-1.0, 0.0, 2.0).rotate(currentAngle*10, 0, 1, 0).rotate(-currentAngle*9, 0, 1, 0.1); //повернуть, переместить фигуру
		gl.uniformMatrix4fv(program.u_ModelMatrix, false, modelMatrix.elements);
		if(program.name == "normalProgram"){
			normalMatrix.setInverseOf(modelMatrix).transpose();
			gl.uniformMatrix4fv(program.u_NormalMatrix, false, normalMatrix.elements)
		...
		modelMatrix.setTranslate(4.0, 0.5, 4.0).rotate(90, 1, 0, 0).rotate(currentAngle, 0, 0, 1)
		.translate(0, 1, 0).rotate(currentAngle*4, 0, 1, 0); //повернуть, переместить фигуру
		gl.uniformMatrix4fv(program.u_ModelMatrix, false, modelMatrix.elements);
		if(program.name == "normalProgram"){
			normalMatrix.setInverseOf(modelMatrix).transpose();
			gl.uniformMatrix4fv(program.u_NormalMatrix, false, normalMatrix.elements)
		...
		modelMatrix.setTranslate(-4.0, 0.5, 4.0).rotate(-90, 1, 0, 0)
		.rotate(-currentAngle*10, 0, 0, 1).translate(0, -1.5, 0); //повернуть, переместить фигуру
		gl.uniformMatrix4fv(program.u_ModelMatrix, false, modelMatrix.elements);
		if(program.name == "normalProgram"){
			normalMatrix.setInverseOf(modelMatrix).transpose();
			gl.uniformMatrix4fv(program.u_NormalMatrix, false, normalMatrix.elements)
		...
		// modelMatrix.setTranslate(0.0, 0.0, -10.0).scale(5,5,5).rotate(90, 1, 0, 0);
